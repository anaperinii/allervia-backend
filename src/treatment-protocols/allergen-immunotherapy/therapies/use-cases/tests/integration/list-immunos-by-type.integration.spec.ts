...
import { ListImmunotherapiesByTypeUseCase } from "../../list-immunotherapies-by-type.use-case";
...
describe('ListImmunotherapiesByTypeUseCase - Integration', () => {
    ...
    it('should list all immunotherapies by type correctly', async () => {
        ...
        const resultAcaros = await listAllByType.execute("Ácaros", authenticatedUser.activeOrgId);
        const resultPolen = await listAllByType.execute("Pólen", authenticatedUser.activeOrgId);
