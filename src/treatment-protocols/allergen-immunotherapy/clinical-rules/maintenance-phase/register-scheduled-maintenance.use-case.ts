...
export class RegisterNextScheduledMaintenanceUseCase {
  ...
  async execute(
    ...
    const nextIntervalDays = await this.calculateNextMaintenanceInterval(
      registeredDose,
      currentUser,
    );
  ...
  private async calculateNextMaintenanceInterval(
    registeredDose: Dose,
    currentUser: AuthenticatedUserPayload,
  ): Promise<number> {
    ...
    const currentIndex = MAINTENANCE_INTERVALS.findIndex(
      (interval) => interval.days === registeredDose.nextIntervalInDays,
    );
