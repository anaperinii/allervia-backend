...
export class MaintenancePhaseService extends IMaintenancePhase {
  constructor(
    private readonly registerScheduledMaintenanceUseCase: RegisterNextScheduledMaintenanceUseCase,
  ) {
    super();
  }

  async registerScheduledMaintenanceDose(
    registeredDose: Dose,
    currentUser: AuthenticatedUserPayload,
    immunotherapy: Immunotherapy,
  ): Promise<Dose> {
    return this.registerScheduledMaintenanceUseCase.execute(
      registeredDose,
      currentUser,
      immunotherapy,
    );
