...
export class BuildUpPhaseService extends IBuildUpPhase {
  ...
  async registerStartingBuildUpDose(
    registeredImmunotherapy: Immunotherapy,
    currentUser: AuthenticatedUserPayload,
    tx?: Prisma.TransactionClient,
  ): Promise<Dose> {
    return await this.registerStartingDoseUseCase.execute(
      registeredImmunotherapy,
      currentUser,
      tx,
    );
  }

  async registerNextScheduledDose(
    registeredDose: Dose,
    currentUser: AuthenticatedUserPayload,
    immunotherapy: Immunotherapy,
  ): Promise<Dose> {
    return await this.registerNextScheduledUseCase.execute(
      registeredDose,
      currentUser,
      immunotherapy,
    );
