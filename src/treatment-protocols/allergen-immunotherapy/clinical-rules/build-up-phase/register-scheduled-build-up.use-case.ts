...
export class RegisterNextScheduledBuildUpUseCase {
  ...
  async execute(
    ...
    if (
      !registeredDose.administeredAt ||
      (registeredDose.status !== 'ADMINISTERED_ON_SCHEDULE' &&
        registeredDose.status !== 'ADMINISTERED_OFF_SCHEDULE')
    ) {
      throw new Error(
        'Apenas doses administradas podem gerar próxima dose agendada',
      );
    }

    if (
      registeredDose.concentration === immunotherapy.targetConcentration &&
    ...
    const { nextConcentration, nextVolume } = await this.calculateNextDose(
      registeredDose,
      currentUser,
      immunotherapy,
    );
  ...
  private async calculateNextDose(
    ...
    if (dosesInCurrentConcentration < 4) {
      ...
      if (
        currentConcentration === immunotherapy.targetConcentration &&
